...
<div class="max-w-7xl mx-auto pt-2">
    ...
    <form method="GET" action="{{ route('admin.patient-records.index') }}" class="mb-3 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="document-item hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 font-medium">{{ $record->patient_number }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    @if($record->bmi)
                                        <div class="font-medium">{{ $record->bmi }}</div>
                                        <div class="text-xs text-gray-500">({{ $record->getBMICategory() }})</div>
                                    @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.patient-records.show', $record->id) }}" class="inline-flex items-center px-2 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-200" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.patient-records.edit', $record->id) }}" class="inline-flex items-center px-2 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200" title="Edit">
        ...
        <div class="md:hidden space-y-3">
            ...
            <div class="document-card bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition duration-200">
                ...
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center flex-1 min-w-0">
                        ...
                        <div class="ml-3 flex-1 min-w-0">
                            <h3 class="text-sm font-semibold text-gray-900 truncate">{{ $record->resident->name ?? 'N/A' }}</h3>
                            <p class="text-sm text-gray-500 truncate">{{ $record->patient_number }}</p>
                            <div class="flex items-center mt-1">
                                @if($record->risk_level)
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $riskColors[$record->risk_level] ?? 'bg-gray-100 text-gray-800' }}">
                                        <i class="fas fa-exclamation-triangle mr-1"></i>
                                        {{ ucfirst($record->risk_level) }} Risk
                                    </span>
                                @endif
                                @if($record->blood_type)
                ...
                <div class="mb-3">
                    @if($record->bmi)
                        <p class="text-sm text-gray-600">
                            ...
                            BMI: <span class="font-medium">{{ $record->bmi }}</span> ({{ $record->getBMICategory() }})
                        </p>
                    @else
                        <p class="text-sm text-gray-600">
                            <i class="fas fa-weight mr-1 text-gray-400"></i>
                            BMI: <span class="text-gray-400">Not calculated</span>
                        </p>
                    @endif
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-100">
                    <a href="{{ route('admin.patient-records.show', $record->id) }}" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 transition duration-200" title="View">
                        <i class="fas fa-eye mr-1"></i>
                        View
                    </a>
                    <a href="{{ route('admin.patient-records.edit', $record->id) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200" title="Edit">
        ...
        <div class="mt-6">
            <nav class="flex items-center justify-between border-t border-gray-200 px-4 sm:px-0">
                <div class="-mt-px flex w-0 flex-1">
                    ...
                    @else
                        <a href="{{ $patientRecords->appends(request()->except('page'))->previousPageUrl() }}" 
                ...
                <div class="hidden md:-mt-px md:flex">
                    @php
                        $currentPage = $patientRecords->currentPage();
                        $lastPage = $patientRecords->lastPage();
                        $startPage = max(1, $currentPage - 2);
                        $endPage = min($lastPage, $currentPage + 2);
                    @endphp
                    
                    @if($startPage > 1)
                        <a href="{{ $patientRecords->appends(request()->except('page'))->url(1) }}" 
                    ...
                    @for($page = $startPage; $page <= $endPage; $page++)
                        ...
                        @else
                            <a href="{{ $patientRecords->appends(request()->except('page'))->url($page) }}" 
                               class="inline-flex items-center border-t-2 border-transparent px-4 pt-4 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700">
                                {{ $page }}
                            </a>
                        @endif
                    ...
                    @if($endPage < $lastPage)
                        ...
                        @endif
                        <a href="{{ $patientRecords->appends(request()->except('page'))->url($lastPage) }}" 
                           class="inline-flex items-center border-t-2 border-transparent px-4 pt-4 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700">
                            {{ $lastPage }}
                        </a>
                    @endif
                ...
                <div class="-mt-px flex w-0 flex-1 justify-end">
                    @if($patientRecords->hasMorePages())
                        <a href="{{ $patientRecords->appends(request()->except('page'))->nextPageUrl() }}" 
            ...
            <div class="mt-4 flex justify-between sm:hidden">
                ...
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700">
                    Page {{ $patientRecords->currentPage() }} of {{ $patientRecords->lastPage() }}
                </span>
            ...
            <div class="mt-4 text-center text-sm text-gray-500">
                Showing {{ $patientRecords->firstItem() }} to {{ $patientRecords->lastItem() }} of {{ $patientRecords->total() }} results
            </div>
        </div>
    @endif
</div>
